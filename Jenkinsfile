pipeline {
    agent any // Ensure this agent has git, docker, aws-cli, kubectl, python, trivy, and sonar-scanner CLI available
    ...
    stages {
        ...
        stage('SonarQube Analysis') { // <<<< NEW STAGE
            steps {
                script {
                    // 'MyLocalSonarQube' should match the name you gave your 
                    // SonarQube server configuration in "Manage Jenkins -> Configure System".
                    // The withSonarQubeEnv wrapper sets environment variables like SONAR_HOST_URL 
                    // and uses the token configured for that server.
                    withSonarQubeEnv('MyLocalSonarQube') {
                        // 'SonarScannerCLI' should match the name you gave the SonarQube Scanner
                        // tool in "Manage Jenkins -> Global Tool Configuration".
                        ...
                        sh "${scannerHome}/bin/sonar-scanner \
                            ...
                            // are read from the sonar-project.properties file in the workspace root.
                    }
                }
            }
        }

        stage('Quality Gate Check') { // <<<< NEW STAGE (Informational for now)
            steps {
                script {
                    // This step requires the SonarQube Scanner for Jenkins plugin to be configured
                    // and a webhook set up in SonarQube to Jenkins for immediate feedback.
                    // For now, we are just logging. A typical step would be:
                    // timeout(time: 1, unit: 'HOURS') { // Timeout for the Quality Gate
                    //     waitForQualityGate abortPipeline: true
                    // }
                    echo "INFO: SonarQube analysis submitted. Quality Gate status can be checked on the SonarQube server."
                    echo "INFO: For automatic pipeline failure based on Quality Gate, configure SonarQube webhooks and use 'waitForQualityGate'."
                }
            }
        }
        ...
        stage('Deploy to EKS') {
            steps {
                script { 
                    ...
                    sh "kubectl rollout status deployment/my-simple-app-deployment --namespace ${env.APP_NAMESPACE} --timeout=3m"
                }
            }
        }
    ...
    post {
        ...
        success {
            echo "Successfully completed all stages for ${env.DOCKER_IMAGE_NAME}:${env.IMAGE_TAG}."
        }
        failure {
