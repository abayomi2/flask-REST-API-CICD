...
# It will also track common metrics like request latency, count, etc. by default for all endpoints.
metrics = PrometheusMetrics(app) 

# Static information about the application
metrics.info('app_info', 'Application info', version='1.0.0', app_name='my-simple-app')


# --- Mock Data ---
# (Your existing mock data remains here)
mock_user_devices = {
    "user123": [
...
PROJECT_TOOLS = [
    ...
    "Prometheus for Metrics Collection", # Added
    "Grafana for Metrics Visualization"  # Added
]

# --- API Endpoints ---

@app.route('/')
# @metrics.do_not_track() # Example: if you don't want to track metrics for this specific route
def hello_world():
    """
    Root endpoint - Serves an HTML page detailing the project.
    """
    version = os.environ.get('APP_VERSION', "1.0")
    # (Your existing HTML content for the homepage remains here)
    html_content = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            h1 {{ color: #f0f0f0; margin-bottom: 10px; font-size: 2.5em; }}
            h2 {{ color: #cfcfcf; margin-top: 30px; margin-bottom: 15px; border-bottom: 2px solid #764ba2; padding-bottom: 5px; font-size: 1.8em; }}
            ...
            li {{ background-color: rgba(255, 255, 255, 0.1); margin-bottom: 10px; padding: 12px 18px; border-radius: 8px; transition: transform 0.2s ease-in-out, background-color 0.2s ease-in-out; }}
            li:hover {{ transform: translateX(5px); background-color: rgba(255, 255, 255, 0.2); }}
            .footer {{ margin-top: 30px; font-size: 0.9em; color: #cccccc; }}
    ...
    html_content += """
            ...
            <div class="footer">
                ...
                <p>Explore the API endpoints: 
                    ...
                    <a href="/api/v1/devices/user123" target="_blank">/api/v1/devices/user123</a> |
...
def request_software(device_id):
    if device_id not in mock_device_status:
        return jsonify({"error": "Device not found"}), 404
    ...
    request_id = f"req_{len(mock_software_requests) + 1}"
